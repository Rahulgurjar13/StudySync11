...
import { TrendingUp, Flame, Clock, Award, ChevronLeft, ChevronRight, Target, Zap } from "lucide-react";
...
export const FocusStreakCalendar = () => {
  const { user } = useAuth();
  ...
  const generateMockData = (): FocusDay[] => {
    ...
    const stored = localStorage.getItem(`focus_data_${year}_${month}`);
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const dayData = storedData[dateStr] || {
        focusMinutes: date <= new Date() ? Math.floor(Math.random() * 240) : 0,
      ...
      data.push({
        ...
        achieved: dayData.focusMinutes >= 120,
      });
    }
  ...
  const getDaysInMonth = () => {
    ...
    // Adjust for Monday start (0 = Sunday, make Monday = 0)
            ...
            {getDaysInMonth().map((day, index) => {
              ...
              return (
                <div
                  ...
                  title={`${dayDate.getDate()} - ${day.focusMinutes} minutes`}
                >
                  {/* Fire icon for achieved days */}
                  {intensityLevel === 5 ? (
